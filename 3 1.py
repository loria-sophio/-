...
def choose_word():
    # სიტყვების სია, საიდანაც პროგრამა შემთხვევით ირჩევს ერთს
    words = ["პითონი", "კომპიუტერი", "თამაში", "სათამაშო", "სწავლა", "პროგრამა", "ჰანგმენი", "ცოდნა"]
    return random.choice(words)


def display_word(word, guessed_letters):
    # აჩვენებს სიტყვას ქვედა ტირეების გამოყენებით და გამოვლენილ ასოებს
...
def hangman():
    word = choose_word()
    guessed_letters = set()
    attempts_left = 6  # მოთამაშეს აქვს 6 მცდელობა
    guessed = False

    print(word)  # სადემონსტრაციოდ დავპრინტე
